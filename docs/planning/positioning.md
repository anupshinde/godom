...
2. Developers bring a browser-native mental model where components live on pages

## What godom actually is

godom is a rendering bridge. Go owns the state. The browser is just a display.

The key mental model shift: **components are not on a page. Components exist in the Go process. Their views exist as virtual DOM in Go memory, and are replicated to the browser.**
...
In godom:
- Components are Go structs. They exist from `Register()` until the process exits.
...
- "Navigation" between pages is just changing which component views the browser is displaying.

## Misperceptions this causes

| What developers expect | What's actually happening |
|---|---|
| "Component loads when I navigate to the page" | Component already exists. Browser requests its current view on connect. |
| "Component unloads when I leave the page" | Component still exists. Browser just stops displaying it. |
| "I need OnMount to initialize data" | Component is initialized when you create it in Go. Browser connect is just rendering. |
| "I need conditional mounting" | The component always exists. Use g-if/g-show to control what the browser displays. |
| "Page navigation loses state" | State is in Go, never lost. The browser just re-requests the view. |
| "I need SPA routing" | You own the HTTP server. Routing is yours. godom renders components, not pages. |

## What needs to happen

### Documentation
- [ ] Explain the mental model explicitly: components exist in Go, browser is a display
...
### Examples
- [ ] Rebalance examples: more SetMux + multi-component patterns, less QuickServe-only
...
### Upfront framing
- [ ] Add a one-liner at the top of README/docs that sets the mental model immediately. Something like: "godom is a server-side component library for rendering Go state in a browser — it is not a web application framework." Right now you have to read between the lines to understand what godom isn't.

### Patterns page
- [ ] Create a "patterns" page showing the 3-4 things developers reach for immediately:
  - Shared state between components (embedded struct pointers)
  - Conditional display with g-if + g-component
...
- [ ] Component "exists" from Register(). Component "renders" when a browser displays it.
...
- "Where's OnMount?" — There's no client-side component lifecycle. Your Go code runs when you want it to.
- "Where's component communication?" — Your Go structs can share pointers. It's just Go.
