...
- **All component state access must go through the event queue.** Each component has a buffered `EventCh` channel and a single `processEvents` goroutine. Browser events, background refreshes, and connection inits are all serialized through this queue. Never call `handleNodeEvent`, `handleMethodCall`, `executeRefresh`, or `handleInit` directly from outside the processor goroutine — always send an event to `EventCh`. The only exception is `findComponentByNodeID`, which still uses `ci.Mu` to read the tree from the WS read loop. See `processEvents` in `internal/server/server.go`.
