...
## Critical invariants
- **IDCounter must never reset.** Each VDOM node gets a unique integer ID from a single `IDCounter` shared by every island in the engine (wired in `Run()` at `internal/server/server.go`). Two reasons IDs must stay unique and monotonically increasing: (1) each target context in the bridge owns a private `nodeMap[id] → DOM node` that accumulates entries over the connection's lifetime, so within an island, resetting the counter makes new subtrees reuse IDs of existing nodes and silently corrupts that nodeMap; (2) across islands, the server's `findIsland` (`internal/server/nodelookup.go`) walks every island's tree looking up a browser-supplied nodeID — if two islands minted overlapping IDs, browser events would be routed to the wrong island. Don't reset the counter (e.g. `ci.IDCounter = &vdom.IDCounter{}` in `BuildUpdate`). See `TestIDCounter_MustOnlyIncrement` in `internal/server/server_test.go`.
