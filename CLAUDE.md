...
- **All component state access must go through the event queue.** Each component has a buffered `EventCh` channel and a single `processEvents` goroutine. Browser events and background refreshes are serialized through this queue. Never call `handleNodeEvent`, `handleMethodCall`, or `executeRefresh` directly from outside the processor goroutine — always send an event to `EventCh`. Two exceptions use `ci.Mu` directly: `findComponentByNodeID` (reads the tree from the WS read loop) and `handleInit` (writes the tree on new connection, must be synchronous to preserve mount order). See `processEvents` in `internal/server/server.go`.
