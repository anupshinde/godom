...
### Embedded mode (per-component badge)

A static HTML page with two godom components (clock and controls). Kill the server to see disconnect badges appear next to each component.

## Customization

By default this example loads custom HTML from `ui/partials/`:
- `disconnect.html` — full-page overlay (root mode)
- `disconnect-badge.html` — per-component badge (embedded mode)
